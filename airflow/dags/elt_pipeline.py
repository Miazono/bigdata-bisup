...
def generate_mock_data():
    ...
    pg_hook.insert_rows(
        table="raw.clients",
        rows=clients,
        target_fields=["name", "registration_date"],
        commit_every=1000,
    )

    records = pg_hook.get_records("SELECT id FROM raw.clients")
    client_ids = [row[0] for row in records]
    ...
    pg_hook.insert_rows(
        table="raw.orders",
        rows=orders,
        target_fields=["client_id", "amount", "order_date"],
        commit_every=1000,
    )
